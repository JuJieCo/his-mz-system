...
public class RoleDaoImpl extends BaseJdbcDao {
	...
	public List<Role> getAllRoles(Object[] objs,Page page) throws Exception {
		...
		if(page==null){
			return this.getJdbcTemplate().query(sql, list.toArray(), new Role());
		}
		return this.getJdbcTemplate().query(PageUtils.fyPage(sql, list.toArray(), page, this.getJdbcTemplate(), Page.DATABASE_TYPE_MYSQL),list.toArray(), new Role());
	}
